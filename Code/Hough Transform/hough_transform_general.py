...
def gradient(filter_x, filter_y, original_image):
    # Construct gradient image (both magnitude and orientation)
...
#===================================================================
def create_accumulator_line(edge_image):
...
def hough_line(vote_threshold, edge_image):
    accumulator = create_accumulator_line(edge_image)
...
#===================================================================
def create_phi_table(edge_template, gradient_template, template_reference):
    phi_table = [[] for _ in range(360)]

    for row in range(len(edge_template)):
        for col in range(len(edge_template[0])):
            if (edge_template[row][col] != 0):
                gradient_orientation = int(gradient_template[row][col][1])
                y = row - template_reference[1]
                x = col - template_reference[0]
                point_to_reference = np.array([x, y])
                phi_table[gradient_orientation].append(point_to_reference)

    return phi_table
#===================================================================

#===================================================================
def create_accumulator_shape(edge_image, gradient_image, phi_table):
    accumulator = np.zeros((len(edge_image), len(edge_image[0])),dtype=np.uint16)
    # uint16 because votes can exceed 255

    for row in range(len(edge_image)):
        for col in range(len(edge_image[0])):
            if (edge_image[row][col] != 0):
                gradient_orientation = int(gradient_image[row][col][1])
                r_values = phi_table[gradient_orientation]
                for r in r_values:
                    y = row - r[1]
                    x = col - r[0]
                    if ((x >= 0) & (y >= 0) & (x < len(edge_image[0])) & (y < len(edge_image))):
                        accumulator[y][x] += 1  # (row, col)
    return accumulator
#===================================================================
...
if __name__ == "__main__":
    template_img = cv2.imread('.\Stah.png')
    sample_img = cv2.imread('.\Shapes.png')

    template_grayscale = cv2.cvtColor(template_img, cv2.COLOR_BGR2GRAY)
    ...
    blurred_template = gaussian_blur(template_grayscale, filter_kernel)
    ...
    gradient_template = gradient(sobel_filter_x, sobel_filter_y, blurred_template)
    gradient_image = gradient(sobel_filter_x, sobel_filter_y, blurred_img)

    edge_threshold = 150

    edge_template = extract_edges(edge_threshold, gradient_template[:,:,0])
    edge_image = extract_edges(edge_threshold, gradient_image[:,:,0])

    # Create phi table
    template_reference = np.array([61, 61])
    phi_table = create_phi_table(edge_template, gradient_template, template_reference)

    # Create accumulator
    accumulator = create_accumulator_shape(edge_image, gradient_image, phi_table)
    ...
    cv2.imshow('Gradient Orientation', cv2.resize(gradient_image[:,:,1], (550, 550)))
